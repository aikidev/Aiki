...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in aiki\n"
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of aiki addresses\n"
...
Value getchaintips(const Array& params, bool fHelp)
{
    ...
    /* Build up a list of chain tips.  We start with the list of all
       ...
       of another block.  */
    std::set<const CBlockIndex*, CompareBlocksByHeight> setTaiki;
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex)
        setTaiki.insert(item.second);
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex) {
        const CBlockIndex* pprev = item.second->pprev;
        if (pprev)
            setTaiki.erase(pprev);
    }

    // Always report the currently active tip.
    setTaiki.insert(chainActive.Tip());

    /* Construct the output array.  */
    Array res;
    BOOST_FOREACH (const CBlockIndex* block, setTaiki) {
        Object obj;
