...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='aiki.conf'
CONFIGFOLDER='/root/.aiki'
COIN_DAEMON='aikid'
COIN_CLI='aiki-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/aiki-dev/aiki-core.git'
COIN_TGZ='https://github.com/aiki-dev/aiki-core/releases/download/3.0.0/aiki-3.0.0-x86_64-linux-signed.tar.gz'
...
function get_ip() {
  declare -a NODE_aiki
  for aiki in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_aiki+=($(curl --interface $aiki --connect-timeout 2 -s4 icanhazip.com))
  done
